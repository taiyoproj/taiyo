"""Tests for the SolrClient class."""
...
@pytest.mark.asyncio
async def test_ping_success(solr_client: SolrClient, monkeypatch):
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    assert await solr_client.ping() is True


@pytest.mark.asyncio
async def test_ping_failure(solr_client: SolrClient, monkeypatch):
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    assert await solr_client.ping() is False


@pytest.mark.asyncio
async def test_add_single_document(solr_client: SolrClient, monkeypatch, sample_doc):
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    response = await solr_client.add(sample_doc)
    assert response["responseHeader"]["status"] == 0


@pytest.mark.asyncio
async def test_add_multiple_documents(
    solr_client: SolrClient, monkeypatch, sample_docs
):
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    response = await solr_client.add(sample_docs)
    assert response["responseHeader"]["status"] == 0


@pytest.mark.asyncio
async def test_delete_by_id(solr_client: SolrClient, monkeypatch):
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    response = await solr_client.delete(ids=ids)
    assert response["responseHeader"]["status"] == 0


@pytest.mark.asyncio
async def test_delete_by_query(solr_client: SolrClient, monkeypatch):
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    response = await solr_client.delete(query=query)
    assert response["responseHeader"]["status"] == 0


@pytest.mark.asyncio
async def test_search_basic(solr_client: SolrClient, monkeypatch, sample_docs):
    """Test basic search functionality."""

    async def mock_request(*args, **kwargs):
        assert kwargs["params"]["q"] == "title:test"
        # Convert sample_docs to dicts for mock response
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    response = await solr_client.search("title:test", MyDocument)
    assert response.num_found == len(sample_docs)
    assert len(response.docs) == len(sample_docs)
    # Ensure returned docs are SolrDocument instances
    for doc in response.docs:
        assert isinstance(doc, type(sample_docs[0]))
        # Check extra field is present and correct
        assert hasattr(doc, "category")
        assert doc.category in {"books", "electronics"}


@pytest.mark.asyncio
async def test_search_with_facets(solr_client: SolrClient, monkeypatch, sample_docs):
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    response = await solr_client.search("*:*", facet="true", facet_field="category")
...
async def test_search_with_highlighting(
    ...
    monkeypatch.setattr(solr_client.client, "request", mock_request)
    response = await solr_client.search("test", hl="true", hl_fl="title")
    assert response.highlighting == highlights


@pytest.mark.asyncio
async def test_error_handling(solr_client: SolrClient, monkeypatch):
    """Test error handling."""
    ...
    with pytest.raises(SolrError) as exc_info:
        await solr_client.search("invalid:query]")
